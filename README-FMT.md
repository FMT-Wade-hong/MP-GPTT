# FeiMaoTecPlanner V1.0.7（FMTPlanner）使用手冊
...
> 本手冊對應 GitHub 正式發布版 [`FMTPlanner-v1.0.7`](https://github.com/FMT-Wade-hong/MP-GPTT/releases/tag/FMTPlanner-v1.0.7)，包含動態飛行模式、Titan 風格設定頁、發光飛行器圖示、QNH、GPS 狀態與繁體中文遙測介面。
...
| 參數保護 | 完整參數頁面須輸入密碼，並提供密碼變更功能 |
| 精簡介面 | 移除模擬器、說明／關於頁面及初始配置中的選配硬體模組頁面 |

![FMTPlanner 飛行資料主畫面](FMT/ManualImages/flight-data-overview.png)
...
2. **飛行計畫（PLAN）**：建立航點、設定高度、檢查任務及上傳任務。
3. **初始配置（SETUP）**：必要的飛控初始設定。
4. **配置／調試（CONFIG）**：飛控參數與規劃器設定。
...
![限禁航區與航線檢查示意](FMT/ManualImages/mission-airspace-check.png)

> 上圖為功能開發階段的畫面示意，標題列版本格式已在 V1.0.3 更新；空域顏色、航點距離與檢查流程仍相同。
...
1. 進入 **配置／調試（CONFIG）**。
2. 選擇 **Full Parameter List／完整參數列表**。
3. 輸入參數密碼後解鎖頁面。
4. 使用 **Change password／變更密碼**設定新的參數密碼。
