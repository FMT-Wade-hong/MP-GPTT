...
> V1.1.6 新增 P400 設定、頻率表、H420 基礎參數與介面調整。示意圖部分沿用前版，不代表新版工具列；QNH 入口已移除。
...
| 快捷操作 | 工具列提供解鎖／上鎖、空速計歸零與控制來源切換；QNH 入口已移除 |
...
![FMTPlanner V1.1.6 飛行資料主畫面](FMT/ManualImages/v110-flight-data.png)
...
![FMTPlanner V1.1.6 啟動畫面](FMT/Assets/fmt-splash-v111.png)

### 登入畫面

![FMTPlanner V1.1.6 登入畫面](FMT/Assets/fmt-login-v111.png)
...
- 「初始配置 → SIK 數傳設定」提供中文 SiK 本機／遠端設定。先中斷 MP 飛行連線，明確選擇 COM 與鮑率後讀取；寫入、恢復預設與韌體更新仍需確認。詳見 [SiK 操作說明](FMT/SIK-SETTINGS.md)。
...
- 工具列的控制來源開關啟動時預設為「遙控器控制」。只有本機搖桿已建立並啟用時，才開放切換為「導控控制」。
...
「初始配置 → P400 數傳設定」包含 DATA／AT 設定、PicoConfig 備用視窗與頻率表三個分頁。進入 AT 會中斷數傳，請先中斷遙測並在地面安全狀態操作。

- **參數設定**：顯示功能、讀回值、新值選單及手冊預設。預設值僅供參考，不會自動套用；RSSI、未定義的 S154 及未知回覆仍唯讀。
- **S102／S110**：修改前須用 CONFIG 強制進入 9600／8N1 AT 模式，勾選對應選項；AT 期間保持暫時速度，返回 DATA 後才採用新鮑率及格式。
- **S107**：點新值欄開啟遮蔽輸入框，再按寫入／驗證／儲存。留空不改密碼。目前接受 1–16 個英數字及底線、句點、連字號；此字元限制是程式保守規則，非手冊完整字元集。無法讀回驗證原文，須確認配對通訊。
- **頻率表**：可選主表 ATP0／副表 ATP1，各編輯 50 筆 MHz 頻率，支援六位小數、匯入、匯出 CSV 及貼上。
- **10 個組合**：可自行命名、保存及載入；選取組合不會直接覆蓋草稿。本機保存不會写入模組，離開前請保存或匯出。
- **模組寫入**：僅支援 S128=2、S238=1，必須填滿 50 筆。確認後先備份原表，再寫入並比對全部讀回值；只修改選定表，不同步另一表或配對端。
- **失敗處理**：取消、逾時或讀回不符都可能已部分保存，不會自動回復。停止操作、釋放序列埠，重新連線確認；備份位於使用者資料目錄的 `P400FrequencyPlans/backups`。

H420 基礎參數由提供的記錄整理，僅適用相符構型，載入不會自動寫入飛控，也不代表完成另一台飛機的校正或飛行驗證。

詳細流程見 [P400 操作說明](FMT/P400-AT-README.md)及 [H420 基礎參數](FMT/FrameParams/README.md)。本版完成離線與模擬測試，尚未完成實機端到端驗證。
...
![FMTPlanner V1.1.6 任務規劃畫面](FMT/ManualImages/v110-mission-planning.png)
...
V1.1.6 已移除工具列 QNH 入口，舊版示意圖中的按鈕不再顯示。
