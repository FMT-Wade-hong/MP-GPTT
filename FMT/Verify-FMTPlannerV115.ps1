...
Test-Source 'V1.1.5 product version' 'FMT\FmtAuthentication.cs' 'ProductVersion = "1.1.5"'
Test-Source 'V1.1.5 project version' 'MissionPlanner.csproj' '<Version>1.1.5</Version>'
Test-Source 'V1.1.5 assembly version' 'Properties\AssemblyInfo.cs' 'AssemblyFileVersion("1.1.5.0")'
...
Test-Source 'Relay control map action' 'GCSViews\FlightData.cs' 'Text = "接力控制"'
Test-Source 'Embedded relay control panel' 'FMT\FmtRelayControlPanel.cs' 'Text = "接力控制  Relay Control"'
Test-Source 'MAVLink forwarding runtime status' 'Controls\SerialOutputPass.cs' 'MAVLink 轉發已啟用'
Test-Source 'Ground station position registry' 'FMT\FmtGroundStationPositionStore.cs' 'class FmtGroundStationPositionStore'
Test-Source 'Relay station coordination heartbeat' 'FMT\FmtRelayControlService.cs' 'FMT_RELAY_CONTROL_V1'
Test-Source 'Main-station authority broadcast' 'FMT\FmtRelayControlService.cs' 'SendPacket("authority", 0)'
Test-Source 'Relay fail-closed joystick gate' 'MainV2.cs' 'FmtRelayControlService.CanLocalStationTransmitControl'
Test-Source 'Station-aware MAVLink write gate' 'Controls\SerialOutputPass.cs' 'CanAcceptMavlinkWriteFrom'
Test-Source 'Relay station forwarding assignment' 'Controls\SerialOutputPass.cs' 'Name = "RelayStation"'
Test-Source 'Main-only control revocation' 'FMT\FmtRelayControlPanel.cs' 'revokeControl.Visible = isMainStation'
Test-Source 'Relay skips duplicate parameter list' 'MainV2.cs' 'skipping duplicate full parameter download'
Test-Source 'Main-only RC_OPTIONS control' 'MainV2.cs' '控制來源由 1 號主站統一管理'
Test-Source 'Takeover request warning color' 'FMT\FmtRelayControlPanel.cs' 'pendingRequest.ForeColor = pending > 1 || localRequestPending ? Color.Gold'
Test-Source 'Receiver-first startup state' 'MainV2.cs' 'FmtControlSourceState = 1'
Test-Source 'Joystick readiness gate' 'MainV2.cs' 'FmtGcsControlButton.Enabled = canChange && joystickReady'
Test-Source 'Live joystick PWM output' 'MainV2.cs' 'ReadFmtLiveJoystickPwm(1)'
Test-Source 'ArduPilot GCS system-id alignment' 'MainV2.cs' 'TryAlignFmtGcsSystemId'
Test-Source 'RC override transmission diagnostics' 'MainV2.cs' 'FMT RC override transmitting'
Test-Source 'Unassigned controller recovery' 'MainV2.cs' 'FmtRelayControlService.ActiveStation == 0'
Test-Source 'Control handover axis stability' 'FMT\FmtControlHandoverDialog.cs' 'RequiredStableMilliseconds'
Test-Source 'Confirmed forced control handover' 'FMT\FmtControlHandoverDialog.cs' '確定要強制切換為導控控制嗎'
Test-Source 'Critical RC option scan' 'FMT\FmtControlHandoverDialog.cs' 'GetConfiguredCriticalSwitches'
Test-Source 'Disabled fence action write guard' 'FMT\FmtSafetySettingsPanel.cs' 'GetWriteBlockedReason'
...
if (Test-Path -LiteralPath $exe -PathType Leaf) {
    ...
    if ($version -eq '1.1.5.0') { Write-Host 'PASS executable version' -ForegroundColor Green }
...
if ($failures.Count -gt 0) {
    throw ('V1.1.5 verification failed: ' + ($failures -join ', '))
}

Write-Host 'FMTPlanner V1.1.5 verification passed.' -ForegroundColor Cyan
