...
{
    // Message-row presentation adapted from TitanPlanner for FeiMaoTecPlanner V1.
    public class MessagesList : UserControl
    {
        ...
        private void DrawMessageRow(Graphics g, (DateTime time, string message, byte severity) msg, int y, int index, bool isSelected, bool isHovered)
        {
            ...
            // FeiMaoTecPlanner V1 adds a green success state while retaining TitanPlanner's
            // red critical, yellow warning, blue notice, and neutral info rows.
            if (IsSuccessMessage(msg.message, msg.severity))
            {
                bgColor = Color.FromArgb(20, 52, 35);
                textColor = Color.FromArgb(135, 235, 170);
            }
            ...
            // Draw severity badge
            int severityX = 70;
            Color badgeColor = IsSuccessMessage(msg.message, msg.severity)
                ? Color.FromArgb(25, 135, 70)
                : GetSeverityBadgeColor(msg.severity);
        ...
        private static bool IsSuccessMessage(string message, byte severity)
        {
            if (string.IsNullOrWhiteSpace(message) || severity < (byte)MAV_SEVERITY.NOTICE)
                return false;

            string text = message.ToLowerInvariant();
            if (text.Contains("not ready") || text.Contains("failed") || text.Contains("failure"))
                return false;

            string[] successTerms =
            {
                " cleared", "clear complete", " complete", "completed", " received",
                " ready", "success", "succeeded", " accepted", " restored"
            };

            foreach (string term in successTerms)
            {
